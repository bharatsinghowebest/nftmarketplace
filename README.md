![NFT.js Logo](https://storage.googleapis.com/nft-static/nft-js-logo.png "NFT.js Logo")
...
A JavaScript library for crypto-native ecommerce: buying, selling, and bidding on any cryptogood. With NFT.js, you can easily build your own native marketplace for your non-fungible tokens, or NFTs. These can be ERC-721 or ERC-1155 (semi-fungible) items. You don't have to deploy your own smart contracts or backend orderbooks.

Published on [GitHub](https://github.com/bharatsinghowebest/nftmarketplace) and [npm](https://www.npmjs.com/package/nft-js)
...
- [Advanced](#advanced)
  ...
  - [Sharing Sale Fees with NFT](#sharing-sale-fees-with-nft)
...
This is the JavaScript SDK for [NFT](https://nft.io), the largest marketplace for crypto collectibles. It allows developers to access the official orderbook, filter it, create buy orders (**offers**), create sell orders (**auctions**), create collections of assets to sell at once (**bundles**), and complete trades programmatically.
...
To get started, create a new NFTJS client, called an NFTPort 🚢, using your Web3 provider:
...
const seaport = new NFTPort(provider, {
...
In a browser with web3 or an extension like [MetaMask](https://metamask.io/) or [Dapper](http://www.meetdapper.com/), you can use `window.ethereum` (or `window.web3.currentProvider` for legacy mobile web3 browsers) to access the native provider. In a Node.js script, you can follow [this example](https://github.com/ProjectNFT/nft-creatures/blob/master/scripts/sell.js) to use a custom mnemonic.

### Fetching Assets

Assets are items on NFT. They can be non-fungible (conforming to standards like ERC721), semi-fungible (like ERC1155 assets), and even fungible (ERC20).
...
The `Asset` type is the minimal type you need for most marketplace actions. `WyvernSchemaName` is optional. If omitted, most actions will assume you're referring to a non-fungible, ERC721 asset. Other options include 'ERC20' and 'ERC1155'. You can import `import { WyvernSchemaName } from "nft-js/lib/types"` to get the full range of schemas supported.

You can fetch an asset using the `NFTAPI`, which will return an `NFTAsset` for you (`NFTAsset` extends `Asset`):

```TypeScript
const asset: NFTAsset = await seaport.api.getAsset({
  tokenAddress, // string
...
When you make an offer on an item owned by an NFT user, **that user will automatically get an email notifying them with the offer amount**, if it's above their desired threshold.
...
When you bid on multiple assets, an email will be sent to the owner if a bundle exists on NFT that contains the assets. In the future, NFT will send emails to multiple owners if the assets aren't all owned by the same wallet.

#### Bidding on ENS Short Name Auctions

The Ethereum Name Service (ENS) is auctioning short (3-6 character) names that can be used for labeling wallet addresses and more. Learn more on the [ENS FAQ](https://nft.io/ens).
...
To create a presale or crowdsale and create batches of sell orders for a single asset factory, first follow the [tutorial](https://docs.nft.io/docs/nft-initial-item-sale-tutorial) for creating your crowdsale contract.
...
Here's an [example script](https://github.com/ProjectNFT/nft-creatures/blob/master/scripts/sell.js) you can use to mint items.
...
Games using this method include [Coins & Steel](https://nft.io/assets/coins&steelfounderssale) and a couple in stealth :) If you have questions or want support, contact us at contact@nft.io (or in [Discord](https://discord.gg/ga8EJbv)).

### Fetching Orders

To retrieve a list of offers and auction on an asset, you can use an instance of the `NFTAPI` exposed on the client. Parameters passed into API filter objects are underscored instead of camel-cased, similar to the main [NFT API parameters](https://nftmarketplace.xyz/v1.0/reference):

```JavaScript
import { OrderSide } from 'nft-js/lib/types'
...
To learn more about signatures, makers, takers, listingTime vs createdTime and other kinds of order terminology, please read the [**Terminology Section**](https://docs.nft.io/reference#terminology) of the API Docs.

The available API filters for the orders endpoint is documented in the `OrderJSON` interface below, but see the main [API Docs](https://docs.nft.io/reference#reference-getting-started) for a playground, along with more up-to-date and detailed explanantions.
...
A handy feature in NFT.js is the ability to transfer any supported asset (fungible or non-fungible tokens) in one line of JavaScript.
...
To transfer fungible assets without token IDs, like ERC20 tokens, you can pass in an `NFTFungibleToken` as the `asset`, set `schemaName` to "ERC20", and include `quantity` in base units (e.g. wei) to indicate how many.
...
For more information, check out the documentation for WyvernSchemas on https://projectnft.github.io/nft-js/.

## Affiliate Program

NFT.js allows to you easily create an affiliate program in just a few lines of JavaScript! It's the crypto-equivalent of bounty hunting, and best of all, it's **fully paid for by NFT** so you can keep all of your winnings 💰
...
You can instantly create an affiliate program for your assets by just passing in one more parameter when fulfilling orders... **and NFT will pay for it!** Whenever someone refers a sale or the acceptance of an offer, you can add a `referrerAddress` to give their wallet credit:
...
As long as the referrer hasn't referred the buyer before, NFT will send the referrer an email congradulating them, along with **1%** of the item's sale price. If you'd like to be able to refer the same user for multiple purchases, contact us at contact@nft.io (or in [Discord](https://discord.gg/ga8EJbv)).
...
When buyers place offers or bids on an asset, the referrers will automatically be recorded on NFT.io. Then, you can use the [Orderbook API](https://docs.nft.io/reference#retrieving-orders) to inspect the `metadata` for orders and manually pay out referrers if you want to. The referrer will be labeled as `referrerAddress` in the `metadata` field.

### Custom Referral Bounties

Sellers can customize the bounties they add to their items when listing them for sale. By default, NFT will pay referrers 1% and sellers pay them nothing, but sellers can increase this up to the full NFT fee (currently 2.5% for most assets) for both assets and bundles:
...
// The final bounty will be 10% + 1% from NFT, or 11 ETH!
...
**NOTE:** The final bounty in the example above will be 10% from the seller plus 1% from NFT, or 11 ETH in total!

Developers can request to increase the NFT fee to allow for higher bounties - by default, it's capped at 2.5%. If you have any questions, contact us at contact@nft.io (or in [Discord](https://discord.gg/ga8EJbv)), or join the program at https://nft.io/account#referrals.

## Advanced

Interested in purchasing for users server-side or with a bot, making bundling items together, scheduling future orders, or making bids in different ERC-20 tokens? NFT.js can help with that.
...
A handy feature in NFT.js is the ability to transfer multiple items at once in a single transaction. This works by grouping together as many `transferFrom` calls as the Ethereum gas limit allows, which is usually under 30 items, for most item contracts.
...
* And tons more! See the "Currencies" list in the sidebar on https://nft.io/assets for a full list, or contact us to add yours: [Discord](https://discord.gg/ga8EJbv)

### Private Auctions

Now you can make auctions and listings that can only be fulfilled by an address or email of your choosing. This allows you to negotiate a price in some channel and sell for your chosen price on NFT, **without having to trust that the counterparty will abide by your terms!**
...
### Sharing Sale Fees with NFT

We share fees for successful sales with game developers, relayers, and affiliates using the NFT orderbook. Developers can customize the fee amount to apply to  buyers and/or sellers.

See [Affiliate Program](#affiliate-program) above for how to register referrers for sales.

More information will appear here when our redesigned affiliate program is ready. In the meantime, contact us at contact@nft.io (or in [Discord](https://discord.gg/ga8EJbv)), or use our legacy affiliate program at https://nft.io/account#referrals.
...
Our recommendation is that you "forward" NFT events to your own store or state management system. Here's an example of doing that with a Redux action:

```JavaScript
import { EventType } from 'nft-js'
...
Auto-generated documentation for each export is available [here](https://projectnft.github.io/nft-js/).

If you need extra help, support is free! Contact the NFT devs. They're available every day on [Discord](https://discord.gg/XjwWYgU) in the `#developers` channel.

### Example Code

Check out the [Ship's Log](https://github.com/ProjectNFT/ships-log), built with the SDK, which shows the recent orders in the NFT orderbook.

You can view a live demo [here](https://ships-log.herokuapp.com/)! Also check out the [Mythereum marketplace](https://mythereum.io/marketplace), which is entirely powered by NFT.js.
...
Note that the tests require access to both Infura and the NFT API. The timeout is adjustable via the `test` script in `package.json`.

**Generate Documentation**

Generate html docs, also available for browsing [here](https://projectnft.github.io/nft-js/):
```bash
npm run docsHtml
```
...
* Are you attempting to purchase a token that's unpurchasable on [NFT](https://nft.io/)?  If so, contact us [Discord](https://discord.gg/XjwWYgU) in the `#developers` channel and we'll help to diagnose the issue.
