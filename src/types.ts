 ...
 * Given by the asset_contract_type in the NFT API
 */
...
// Wyvern Schemas (see https://github.com/ProjectNFT/wyvern-schemas)
...
/**
 * The NFT account object appended to orders, providing extra metadata, profile images and usernames
 */
export interface NFTAccount {
  // Wallet address for this account
  address: string
  // Public configuration info, including "affiliate" for users who are in the NFT affiliate program
  config: string

  // This account's profile image - by default, randomly generated by the server
  profileImgUrl: string

  // More information explicitly set by this account's owner on NFT
  user: NFTUser | null
}
...
/**
 * Annotated asset contract with NFT metadata
 */
export interface NFTAssetContract extends NFTFees {
  ...
  description: string
  // Contract's Etherscan / NFT symbol
  tokenSymbol: string
...
/**
 * Annotated collection with NFT metadata
 */
export interface NFTCollection extends NFTFees {
...
/**
 * Annotated asset spec with NFT metadata
 */
export interface NFTAsset extends Asset {
  assetContract: NFTAssetContract
  ...
  transferFeePaymentToken: NFTFungibleToken | null
}
...
export interface AssetEvent {
  ...
  paymentToken: NFTFungibleToken | null
}
...
/**
 * Full annotated Fungible Token spec with NFT metadata
 */
export interface NFTFungibleToken extends Token {
  imageUrl?: string
  ethPrice?: string
  usdPrice?: string
}
...
// Backwards compat
export type FungibleToken = NFTFungibleToken

/**
 * Bundles of assets, grouped together into one NFT order
 * URLs for bundles are auto-generated from the name
 */
export interface NFTAssetBundle {
  maker: NFTAccount
  assets: NFTAsset[]
  name: string
  slug: string
...
export interface NFTAssetBundleJSON {
  assets: NFTAsset[]
  name: string
  description?: string
  external_link?: string
 ...
 */
export interface NFTAssetBundleQuery extends Partial<NFTAssetBundleJSON> {
...
export interface NFTFees {
  // Fee for NFT levied on sellers
  nftSellerFeeBasisPoints: number
  // Fee for NFT levied on buyers
  nftBuyerFeeBasisPoints: number
 ...
 */
export interface ComputedFees extends NFTFees {
  // Total fees. dev + nft
...
export interface Order extends UnsignedOrder, Partial<ECSignature> {
  ...
  makerAccount?: NFTAccount
  takerAccount?: NFTAccount
  paymentTokenContract?: NFTFungibleToken
  feeRecipientAccount?: NFTAccount
  cancelledOrFinalized?: boolean
  markedInvalid?: boolean
  asset?: NFTAsset
  assetBundle?: NFTAssetBundle
}

/**
 * Order attributes, including orderbook-specific query options
 * See https://docs.nft.io/reference#retrieving-orders for the full
 ...
 * See https://docs.nft.io/reference#retrieving-orders for
 * full docs.
 */
 ...
 */
export interface NFTAssetQuery {
  owner?: string
  asset_contract_address?: string
  token_ids?: Array<number | string>
  search?: string
 ...
 */
export interface NFTFungibleTokenQuery extends Partial<NFTFungibleToken> {
  limit?: number
  offset?: number
...
// Backwards compat
export type FungibleTokenQuery = NFTFungibleTokenQuery
