...
import { ECSignature, FeeMethod, HowToCall, Network, NFTAPIConfig, OrderSide, SaleKind, UnhashedOrder, Order, UnsignedOrder, PartialReadonlyContractAbi, EventType, EventData, NFTAsset, WyvernSchemaName, WyvernAtomicMatchParameters, NFTFungibleToken, WyvernAsset, ComputedFees, Asset, WyvernNFTAsset, WyvernFTAsset, TokenStandardVersion } from './types'
import {
  confirmTransaction,
...
export class NFTPort {
  ...
  public async getFungibleTokens(
    ...
    const offlineTokens: NFTFungibleToken[] = [
  ...
  public async computeFees(
    ...
    let nftBuyerFeeBasisPoints = DEFAULT_BUYER_FEE_BASIS_POINTS
    let nftSellerFeeBasisPoints = DEFAULT_SELLER_FEE_BASIS_POINTS
    ...
    // Check that bounty is in range of the nft fee
    ...
    if (sellerBountyBasisPoints > 0 && bountyTooLarge) {
      ...
      if (maxTotalBountyBPS >= OPENSEA_SELLER_BOUNTY_BASIS_POINTS) {
        errorMessage += ` Remember that NFT will add ${OPENSEA_SELLER_BOUNTY_BASIS_POINTS / 100}% for referrers with NFT accounts!`
      }
    ...
    return {
      ...
      nftBuyerFeeBasisPoints,
      nftSellerFeeBasisPoints,
  ...
  public async _makeBuyOrder(
    ...
    const openSeaAsset: NFTAsset = await this.api.getAsset(asset)
  ...
  private async _atomicMatch(
    ...
    try {
      ...
      throw new Error(`Oops, the Ethereum network rejected this transaction :( The NFT devs have been alerted, but this problem is typically due an item being locked or untransferrable. The exact error was "${error.message.substr(0, MAX_ERROR_LENGTH)}..."`)
    }
