...
suite('api', () => {
  ...
  test('API has correct base url', () => {
    assert.equal(mainApi.apiBaseUrl, 'https://api.nft.io')
    assert.equal(rinkebyApi.apiBaseUrl, 'https://rinkeby-api.nft.io')
  })
  ...
  test('Rinkeby API orders have correct NFT url', async () => {
    const order = await rinkebyApi.getOrder({})
    if (!order.asset) {
      return
    }
    const url = `https://rinkeby.nft.io/assets/${order.asset.assetContract.address}/${order.asset.tokenId}`
    assert.equal(order.asset.nftLink, url)
  })

  test('Mainnet API orders have correct NFT url', async () => {
    const order = await mainApi.getOrder({})
    if (!order.asset) {
      return
    }
    const url = `https://nft.io/assets/${order.asset.assetContract.address}/${order.asset.tokenId}`
    assert.equal(order.asset.nftLink, url)
  })
