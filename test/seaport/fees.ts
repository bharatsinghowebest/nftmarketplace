...
import { NFTPort } from '../../src/index'
import * as Web3 from 'web3'
import { Network, OrderSide, UnhashedOrder, Order, NFTCollection, NFTAsset, NFTAssetBundle, FeeMethod } from '../../src/types'
...
suite('seaport: fees', () => {
  ...
  test("Computes fees correctly for non-zero-fee asset", async () => {
    ...
    const buyerFeeBasisPoints = collection.nftBuyerFeeBasisPoints + collection.devBuyerFeeBasisPoints
    const sellerFeeBasisPoints = collection.nftSellerFeeBasisPoints + collection.devSellerFeeBasisPoints
    ...
    assert.equal(buyerFees.nftBuyerFeeBasisPoints, collection.nftBuyerFeeBasisPoints)
    assert.equal(buyerFees.nftSellerFeeBasisPoints, collection.nftSellerFeeBasisPoints)
    ...
    assert.equal(sellerFees.nftBuyerFeeBasisPoints, collection.nftBuyerFeeBasisPoints)
    assert.equal(sellerFees.nftSellerFeeBasisPoints, collection.nftSellerFeeBasisPoints)
    ...
    assert.equal(heterogenousBundleSellerFees.nftBuyerFeeBasisPoints, DEFAULT_BUYER_FEE_BASIS_POINTS)
    assert.equal(heterogenousBundleSellerFees.nftSellerFeeBasisPoints, DEFAULT_SELLER_FEE_BASIS_POINTS)
    ...
    assert.equal(privateSellerFees.nftBuyerFeeBasisPoints, 0)
    assert.equal(privateSellerFees.nftSellerFeeBasisPoints, 0)
    ...
    assert.equal(privateBuyerFees.nftBuyerFeeBasisPoints, 0)
    assert.equal(privateBuyerFees.nftSellerFeeBasisPoints, 0)
  ...
  test.skip("Computes fees correctly for zero-fee asset", async () => {
    ...
    assert.equal(buyerFees.nftBuyerFeeBasisPoints, 0)
    assert.equal(buyerFees.nftSellerFeeBasisPoints, 0)
    assert.equal(buyerFees.sellerBountyBasisPoints, 0)
    ...
    assert.equal(sellerFees.nftBuyerFeeBasisPoints, 0)
    assert.equal(sellerFees.nftSellerFeeBasisPoints, 0)
  ...
  test("Errors for computing fees correctly", async () => {
    ...
    try {
      ...
      if (!error.message.includes('bounty exceeds the maximum') ||
          !error.message.includes('NFT will add')) {
...
function unitTestFeesBuyOrder({
  ...
  assert.equal(+makerRelayerFee, asset.collection.nftBuyerFeeBasisPoints)
  assert.equal(+takerRelayerFee, asset.collection.nftSellerFeeBasisPoints)
...
export function testFeesMakerOrder(order: Order | UnhashedOrder, collection?: NFTCollection, makerBountyBPS?: number) {
  ...
  if (collection) {
    const totalSellerFee = collection.devSellerFeeBasisPoints + collection.nftSellerFeeBasisPoints
    const totalBuyerFeeBasisPoints = collection.devBuyerFeeBasisPoints + collection.nftBuyerFeeBasisPoints
    // Homogenous sale
    if (order.side == OrderSide.Sell && order.waitingForBestCounterOrder) {
      ...
      assert.equal(order.makerRelayerFee.toNumber(), collection.devSellerFeeBasisPoints + collection.nftSellerFeeBasisPoints)
      // Check bounty
      if (collection.nftSellerFeeBasisPoints >= OPENSEA_SELLER_BOUNTY_BASIS_POINTS) {
        assert.isAtMost(OPENSEA_SELLER_BOUNTY_BASIS_POINTS + order.makerReferrerFee.toNumber(), collection.nftSellerFeeBasisPoints)
      } else {
      ...
      assert.equal(order.makerRelayerFee.toNumber(), collection.devBuyerFeeBasisPoints + collection.nftBuyerFeeBasisPoints)
    }
  } else {
    // Heterogenous
